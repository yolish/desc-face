...
class DescriptorHead(nn.Module):
    def __init__(self,inchannels=512,num_anchors=3, descriptor_dim=64, attentive=True):
    ...
    def forward(self,x):
        x = attention(self.proj_q(x), self.proj_k(x), self.proj_v(x)) #self.interlaces_sparse_self_attention(x, p_h, p_w)
        out = self.conv1x1(x)
        out = out.permute(0,2,3,1).contiguous()

        return out.view(out.shape[0], -1, self.descriptor_dim)

    def interlaces_sparse_self_attention(self, x, p_h, p_w):
        # https://wzwhit.github.io/2019/12/18/Interlaced-Sparse-Self-Attention-for-Semantic-Segmentation/
        ...
        x = attention(self.proj_q(x), self.proj_k(x), self.proj_v(x))
        x = x.reshape(n, q_h, q_w, c, p_h, p_w)
        return x.permute(0, 3, 1, 4, 2, 5).contiguous().reshape(n, c, h, w)

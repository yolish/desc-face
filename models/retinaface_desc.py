...
class DescriptorHead(nn.Module):
    def __init__(self,inchannels=512,num_anchors=3, descriptor_dim=64, attentive=False):
        ...
        self.use_att = attentive
        if self.use_att:
            self.proj_q = nn.Conv2d(inchannels,inchannels,kernel_size=(1,1),stride=1,padding=0)
            self.proj_k = nn.Conv2d(inchannels,inchannels,kernel_size=(1,1),stride=1,padding=0)
            self.proj_v = nn.Conv2d(inchannels,inchannels, kernel_size=(1, 1), stride=1, padding=0)

    def forward(self,x):
        if self.use_att:
            x = attention(self.proj_q(x), self.proj_k(x), self.proj_v(x))
        out = self.conv1x1(x)
...
class DescRetinaFace(nn.Module):
    def __init__(self, cfg = None, phase = 'train'):
        ...
        self.desc_head = self._make_desc_head(fpn_num=3, inchannels=cfg['out_channel'],
                                              descriptor_dim=cfg['descriptor_dim'], attentive=cfg['attention'])
    ...
    def _make_desc_head(self,fpn_num=3,inchannels=64,anchor_num=2, descriptor_dim=64, attentive=False):
        if self.multi_scale_descriptor_head:
            desc_head = nn.ModuleList()
            for i in range(fpn_num):
                desc_head.append(DescriptorHead(inchannels,anchor_num,descriptor_dim, attentive))
        else:

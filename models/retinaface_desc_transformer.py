...
class DescriptorHead(nn.Module):
    def __init__(self,inchannels=512,num_anchors=3, descriptor_dim=64):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=descriptor_dim, nhead=4,
                                                   dim_feedforward=descriptor_dim, activation='gelu')
    ...
    def forward(self,x):
        ...
        if True:
            ...
            out = out.view(out.shape[0], -1, self.descriptor_dim)
            out = self.transformer_encoder(out.permute(1, 0, 2).contiguous())
